...
from colormanager import colors
from menus.loose_menu import LooseMenu
...
class Game:
    ...
    def __init__(self, _screen, color_manager, config_data, _menu):
        ...
        if not self.audio_settings["muted"]:
            self.background_musik = AudioManager(self.audio_settings["game_background"], -1, 1,
                                                 audio_file="music/game.mp3",
                                                 play=True)
            self.effect_music = AudioManager(self.audio_settings["effects"], 0, 2, audio_file="music/level_up.mp3")
        else:
            self.background_musik = AudioManager(0, -1, 1, audio_file="music/game.mp3", play=True)
    ...
    def loop(self):
        ...
        while not self.done:
            ...
            if self.game.state == "gameover":
                self.done = True
                LooseMenu(self.screen, self.game, self.color_manager, self.config_data)
    ...
    def pause_menu(self):
        ...
        options = [Options(self.screen, "Resume", (123, 260), self.color_manager),
                   Options(self.screen, "Retry", (145, 300), self.color_manager),
                   Options(self.screen, "Settings", (123, 340), self.color_manager),
                   Options(self.screen, "Menu", (145, 380), self.color_manager),
                    ...
                    for option in options:
                        if option.rect.collidepoint(pygame.mouse.get_pos()):
                            if option.text == "Resume":
                                ...
                                Settings(self.screen, self.game, self.menu, self.config_data, self.background_musik, self.effect_music, self.color_manager, "game", -50)
